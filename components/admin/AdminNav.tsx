...
export function AdminNav() {
  ...
  // Uses the global 30s staleTime (see providers.tsx) — mutations that
  // change these counts (approve/reject/status-change) explicitly invalidate
  // this query, so it still updates immediately after any action that
  // actually moves the count.
  const { data: counts } = trpc.admin.navCounts.useQuery();
